...
export const verdictBench = internalQuery({
  ...
  handler: async () => {
    ...
    const cases = [
      ...
      { name: "stub call + red recoloured after the call → no-show, colour", input: { ...base, call: { status: "completed", duration: 30 }, recolor: "done" as const, colorId: COLOR.RED }, expect: ["unknown", null] },
      ...
      { name: "no call + red first seen after the call → unknown (red is used for 'didn't close'; a recording decides)", input: { ...base, call: null, recolor: "unverified" as const, colorId: COLOR.RED }, expect: ["unknown", null] },
