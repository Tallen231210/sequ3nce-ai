...
export const verdictBench = internalQuery({
  ...
  handler: async () => {
    ...
    const cases = [
      ...
      { name: "stub call (bot got in, unclassifiable) + red recoloured after the call → unknown", input: { ...base, call: { status: "completed", duration: 30 }, recolor: "done" as const, colorId: COLOR.RED, botGotIn: true }, expect: ["unknown", null] },
      ...
      { name: "no bot got in + red after the call → no-show by colour", input: { ...base, call: null, recolor: "unverified" as const, colorId: COLOR.RED }, expect: ["no_show", "calendar_color"] },
      { name: "bot got in (unclassifiable) + red after the call → unknown (red means 'didn't close' when the bot was there)", input: { ...base, call: null, recolor: "unverified" as const, colorId: COLOR.RED, botGotIn: true }, expect: ["unknown", null] },
      { name: "no call + dark green first seen after the call → showed by colour", input: { ...base, call: null, recolor: "unverified" as const, colorId: COLOR.DARK_GREEN }, expect: ["showed", "calendar_color"] },
