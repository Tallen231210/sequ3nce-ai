    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-[680px] lg:max-w-[960px] mx-auto px-6 pt-10 pb-20">
        ...
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <h1 className="text-[32px] font-bold text-zinc-900 mb-1 leading-tight flex items-center gap-3">
            ...
            {profile.badges?.includes('vip') && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider bg-yellow-100 text-yellow-800 border border-yellow-300">
                <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path d="M2 6l4 3 4-6 4 6 4-3v8a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V6z" /></svg>
                VIP
              </span>
            )}
        ...
        {profile.stats && profile.stats.cashCollected > 0 && (
          <FeaturedStat amount={profile.stats.cashCollected} isVerified={profile.isVerified !== false} isGold={profile.badges?.includes("vip")} />
        )}

        {/* Stats Grid */}
        <StatsGrid stats={profile.stats} isVerified={profile.isVerified !== false} isGold={profile.badges?.includes("vip")} />
