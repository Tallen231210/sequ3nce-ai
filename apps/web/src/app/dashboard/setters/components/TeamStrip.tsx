...
export function TeamStrip({ strip, basis, notes = [] }: { strip: BookingsData["strip"]; basis: string; notes?: string[] }) {
  const rows: Array<{ label: string; hint: string; cell: (c: BookingsData["strip"][number]) => string }> = [
    ...
    { label: "Outcome known", hint: "Finished calls with an outcome (a recording, a person's mark, or a dark green / yellow set after the call) over all finished calls", cell: (c) => (c.due > 0 ? `${int(c.outcomeKnown)} of ${int(c.due)}` : "—") },
