...
const G = {
  ...
  rescheduled: { label: "Rescheduled", hint: "Moved to another time, so the call never happened in this range. Counted here so the outcomes add up to the calls." },
  unknown: { label: "Unknown", hint: "No evidence yet either way. Counted, never assumed." },
  showRate: { label: "Show rate", hint: "Showed over showed plus no-show — shown only when at least half the finished calls have an outcome. The line under it says how many do." },
  ...
  const outbound: CardVM[] = bookings.outbound.map((row) => {
    ...
    return {
      ...
      metrics: [
        ...
        metric("rescheduled", row.rescheduled, "int"),
