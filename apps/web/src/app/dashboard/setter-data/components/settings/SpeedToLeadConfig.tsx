...
export function SpeedToLeadConfig({
  ...
  const [slowThresholdMinutes, setSlowThresholdMinutes] = useState<string>(
    String(settings.slowThresholdMinutes ?? 30),
  );
  ...
  async function handleSave() {
    ...
    try {
      ...
      const thr = parseInt(slowThresholdMinutes, 10);
      if (!Number.isNaN(thr)) {
        args.slowThresholdMinutes = thr;
      }
  ...
  return (
    <Card>
      <CardContent className="p-6">
        <div className="mb-4">
          <h3 className="text-base font-semibold">Speed-to-lead pings</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            Get a Slack or Discord ping the moment a setter dials a brand-new
            lead for the first time. Includes how long the lead waited and the
            team&apos;s historical connect rate at that speed. Off by default —
            opt-in per team.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <input
              id="speedtolead-enabled"
              type="checkbox"
              checked={enabled}
              onChange={(e) => setEnabled(e.target.checked)}
              className="h-4 w-4 rounded border-input"
            />
            <Label htmlFor="speedtolead-enabled" className="cursor-pointer">
              Send per-lead speed-to-lead pings
            </Label>
          </div>
          ...
          {enabled && (
            <>
              <div className="space-y-2">
                <Label htmlFor="speedtolead-threshold">
                  Slow-call badge threshold
                </Label>
                <Select
                  value={slowThresholdMinutes}
                  onValueChange={setSlowThresholdMinutes}
                >
                  <SelectTrigger id="speedtolead-threshold" className="w-56">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Calls faster than this render ✅. Slower renders ⚠️. 3× slower
                  renders 🚨.
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="speedtolead-channel">Send to</Label>
                <Select
                  value={channel}
                  onValueChange={(v) => setChannel(v as "slack" | "discord")}
                >
                  <SelectTrigger id="speedtolead-channel">
              ...
              {channel === "discord" && (
                <div className="space-y-2">
                  <Label htmlFor="speedtolead-discord">
                    Discord webhook URL
                  </Label>
                  <Input
                    id="speedtolead-discord"
                    type="url"
