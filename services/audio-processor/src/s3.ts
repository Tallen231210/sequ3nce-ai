 ...
 */
function createWavHeader(pcmDataLength: number, sampleRate: number, numChannels: number = 2): Buffer {
 ...
 */
function pcmToWav(pcmBuffer: Buffer, sampleRate: number, numChannels: number = 2): Buffer {
  const wavHeader = createWavHeader(pcmBuffer.length, sampleRate, numChannels);
...
export async function uploadRecording(
  ...
  logger.info(`Converting PCM to WAV: ${pcmBufferSize} bytes raw PCM at ${sampleRate}Hz, ${numChannels} channel(s)`);
  const wavBuffer = pcmToWav(audioBuffer, sampleRate, numChannels);
