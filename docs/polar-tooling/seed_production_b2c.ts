// Build the four Sequ3nce Personal (B2C) plan products in Polar's PRODUCTION.
//
// Run: npx tsx docs/polar-tooling/seed_sandbox_b2c.ts
//
// Sandbox only, by construction — reads ~/.polar-key and posts to
// api.polar.sh (same guarantee as seed_sandbox.ts).
//
...
if (process.argv[2] !== "--yes-production") {
  console.error("This seeds PRODUCTION Polar. Run with --yes-production to confirm.");
  process.exit(1);
}
